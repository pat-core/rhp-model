...
class RHP:
    def __init__(self, evaporator_temp, condenser_temp):
        ...
        self.X=np.concatenate([self.Xc, self.Xa[1:-1], self.Xe])
        self.DX=diff(self.X)
        ...
        self.alpha=zeros(self.N-1) # taper angle
        self.alpha[0:self.Nc-1]=atan(self.Riae-self.Ric)/self.Lc
        self.Ri=self.Ric + np.concat([[0], cumsum(self.DX*sin(self.alpha))])
    ...
    def __str__(self):
        ...
        return (f"Model of a rotating heat pipe [Song2003]\n"
                f"Filling Ratio: TODO\n"
                f"Evaporator Temperature: {self.TE} K\n"
                f"Condenser Temperature: {self.TC} K\n")
        
        
    def reynolds(self, d,U):
        #REYNOLDS 
        # Reynolds Number of liquid flow
        # d(elta)...film thickness
        # U...average liquid velocity
        
        Re=abs(4*U*d/self.nul)
        return Re
        
        
    def grashof(self, rim, a, dTm, deltam):
        #GRASHOF
    ...
    def restart_values(self, N):
            #RESTART_VALUES 
            # generate list of factors

            K=zeros(N+1)# last value must be zero, but is not used for iterations
            N2=int(floor(N/2))
            N21=N2+1
    ...
    def liquid_volume(self, d, RI):
        #LIQUID_VOLUME
        # d...film heigth in condenser (nodal)
        # RI...inner radius (mid-point)

        deltamid=0.5*d[:-1] + 0.5*d[1:]    
        V= 2*pi*sum( deltamid*self.DX*RI)

        return V
    
    
    def look_up_song(self, d2D):
        #LOOK_UP_SONG 
        ...
        ml=interp( d2D, d2D_data, ml_data)

        return ml


    def liquid_velocities(self, d0, d1, mt0, dmt, ri0, dx, a, b):
        #LIQUID_VELOCITIES 
        ...
        # vapour velocity
        uv0=mt0/(self.rhov*pi*ri0**2)# stationary: mass flow forward (liquid)=mass flow backward (vapour)
    ...
    def heat_transfer_model_film_condensation(self, d0,d1, Two, Tsat, dx,ri,ro, opt_arg=0 ):
        #FILM_CONDENSATION
        ...
        qw=self.kw*(Twi-Two)/ln;   # qw is always determined in contrast to ql 
        dmt=qw*2*pi*ri*dx/self.hfg
        
        return [dmt, Twi, qw]
    
    
    def heat_transfer_model_mixed_convection(self, d0,d1, Two, Tsat, dx,ri,ro, a ):
        #MIXED_CONVECTION 
        # solve liquid flow model for difference in mass flow (liquid) 
        ...
        Gr=self.grashof(ri, a, Tsat-Two, d)  # here use Two
        Ra=Gr*self.Pr
        Nun=0.133*Ra**0.375     # Nusselt Number of natural convection..
        Num=(Nuf**(7/2)+Nun**(7/2))**(2/7)    # ..and mixed 
        if (d>0):  # there is a liquid layer
            Twi=((Num*self.kl/d)*Tsat+(self.kw/ln)*Two)/(self.kw/ln+Num*self.kl/d)
        else:   # there is no liquid layer
            Twi=Tsat

        # second run with Twi=Twi_first run
        Gr=self.grashof(ri, a, Tsat-Twi, d)  # here use Twi 
        Ra=Gr*self.Pr
        Nun=0.133*Ra**0.375     # Nusselt Number of natural convection..
        Num=(Nuf**(7/2)+Nun**(7/2))**(2/7)    # ..and mixed 
        if (d>0):  # there is a liquid layer
    ...
    def liquid_flow_model_film_condensation(self, d0,d1,mt1, dx,a,ri0, uv,uld, op1=0, op2=2 ):
        #FILM_CONDENSATION
        # solve liquid flow model for difference in mass flow (liquid) 
        # evaluation at begin x=x0 (left) of FV using uv and uld from x1 (right)

        # solve for dmt=Z/N, keep in mind mt1=mt0+dmt
        Z1=mt1-(self.rhol/self.mul)*(self.omega**2)*ri0*(sin(a)-cos(a)*(d1-d0)/dx)*(1/3)*(d0**3)*2*pi*ri0*self.rhol
        N1=1-(1/2)*((d0**2)/(self.mul*dx))*(uv*cos(a)+uld)*2*pi*ri0*self.rhol
        dmt=Z1/N1
    ...
    def liquid_flow_model_mixed_convection(self, d0,d1,mt1, dx,a,ri0, uv1, uld1, Twi, Tsat ):
        #MIXED_CONVECTION 
        # solve liquid flow model for difference in mass flow (liquid) 
        ...
        _,_,u_avg = self.liquid_velocities(d0, d1, mt1+dmt, dmt, ri0, dx, a, b_forced)

        # characteristic numbers (fixed values at mid-point, iterated values right)
        Gr=self.grashof(rim, a, dTm, deltam)  
        Re=self.reynolds(deltam, u_avg)# u_avg is at x1 
        Ra=Gr*self.Pr
        GrRe2=Gr/(Re**2)
        ...
        if d1>0:
            U1=mt1*(b+1)/(2*pi*ri1*self.rhol*d1)
        else:
            U1=0# no liquid, no velocity
        
        # momentum bilance
        QP = 0.4e1 * ((b + 1) * Av ** 2 * ((Cfw * dx * b ** 2) + (0.3e1 / 0.2e1 * Cfw * dx - d0 + d1) * b + (Cfw * dx) / 0.2e1) * self.rhov + 0.4e1 * Cfd * rim ** 2 * self.rhol * dx * (b + 0.1e1 / 0.2e1) * pi ** 2 * deltam ** 2) * U1 / (0.2e1 * (b + 1) * ((Cfw * dx * b ** 2) + (0.3e1 / 0.2e1 * Cfw * dx - d0 + d1) * b + (Cfw * dx) / 0.2e1 + 0.2e1 * deltam) * Av ** 2 * self.rhov + 0.8e1 * Cfd * rim ** 2 * self.rhol * dx * (b + 0.1e1 / 0.2e1) * pi ** 2 * deltam ** 2)
        QQ = (-0.16e2 * rim * Av ** 2 * self.omega ** 2 * ((b + 1) ** 2) * (b + 0.1e1 / 0.2e1) * self.rhov * (-d1 + d0) * deltam * cos(a) - 0.16e2 * rim * Av ** 2 * self.omega ** 2 * dx * ((b + 1) ** 2) * (b + 0.1e1 / 0.2e1) * self.rhov * deltam * sin(a) + 0.2e1 * (Av ** 2 * (Cfw * dx * (b ** 2) + (0.3e1 / 0.2e1 * Cfw * dx - d0 + d1) * b + Cfw * dx / 0.2e1 - 0.2e1 * deltam) * (b + 1) * self.rhov + 0.4e1 * Cfd * rim ** 2 * self.rhol * dx * (b + 0.1e1 / 0.2e1) * pi ** 2 * deltam ** 2) * U1 ** 2) / (0.2e1 * (b + 1) * (Cfw * dx * (b ** 2) + (0.3e1 / 0.2e1 * Cfw * dx - d0 + d1) * b + Cfw * dx / 0.2e1 + 0.2e1 * deltam) * Av ** 2 * self.rhov + 0.8e1 * Cfd * rim ** 2 * self.rhol * dx * (b + 0.1e1 / 0.2e1) * pi ** 2 * deltam ** 2)
        ...
        mt0=U0*(2*pi*ri0*self.rhol*d1)/(b+1)
        dmt=mt1-mt0

        U0=0.07461# result from film condensation

        # debug info
        tauv = Cfd * self.rhol ** 2 * pi ** 2 * rim ** 2 * (U0 + U1) ** 2 * deltam ** 2 / self.rhov / ((b + 1) ** 2) / Av ** 2 / 0.2e1
        tauw = Cfw * self.rhol * (U0 + U1) ** 2 / 0.8e1
        Il = self.omega ** 2 * (sin(a) * dx + (-d1 + d0) * cos(a)) * rim * deltam / dx
...
print(song_rhp.grashof(0.05, 30, 10, 0.002))  # Example usage of the grashof function
print(song_rhp.restart_values(30))  # Example usage of the restart_values function
print(song_rhp.liquid_volume(np.ones(50), np.ones(49)))  # Example usage of the liquid_volume function
print(song_rhp.look_up_song(0.01))  # Example usage of the look_up_song function
print(song_rhp.liquid_velocities(0.002, 0.001, 0.01, 0.005, 0.025, 0.1, 0.5, 1))  # Example usage of the liquid_velocities function
print(song_rhp.heat_transfer_model_film_condensation(1,2,3,4,5,6,7))  # Example usage of the liquid_flow_model_film_condensation function
